...
class DrawView: UIView {
    ...
    func graph(vertex: CGPoint, item: UserData, index: Int) {
        ...
        if friendListItems[0].id == item.id || friendListItems[1].id == item.id || friendListItems[2].id == item.id {
            let imageButton = UIButton()
            ...
            if index == 0 {
                imageButton.tintColor = Asset.Colors.gold.color
            } else if index == 1 {
                imageButton.tintColor = Asset.Colors.silver.color
            } else if index == 2 {
                imageButton.tintColor = Asset.Colors.bronze.color
            }

            imageButton.transform = imageButton.transform.rotated(by: 30 / 180 * .pi)
